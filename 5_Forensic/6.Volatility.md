...
* **`iehistory`**: Trích xuất lịch sử duyệt web internet.
  * *Lệnh*: `volatility -f memdump.mem --profile=PROFILE iehistory` 
* **`filescan`**: Xác định bất kỳ tệp nào trên hệ thống từ hình ảnh bộ nhớ (danh sách có thể cực kỳ dài).
  * *Lệnh*: `volatility -f memdump.mem --profile=PROFILE filescan` [cite: 165]
* **`dumpfiles`**: Truy xuất tệp từ hình ảnh bộ nhớ. Bạn cần chọn vị trí trích xuất tệp.
  * *Lệnh*: `volatility -f memdump.mem --profile=PROFILE dumpfiles -n --dump-dir=./` 

---

## 4. Tài Nguyên Bổ Sung
* **Tải file bộ nhớ mẫu:** `https://github.com/volatilityfoundation/volatility/wiki/Memory-Samples` 
* **Danh sách lệnh hữu ích:** `https://book.hacktricks.xyz/forensics/volatility-examples/list-processes` 
* **Bài viết tham khảo:** "First steps to volatile memory analysis" trên Medium bởi P4N4Rd3.